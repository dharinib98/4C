...
void BeamInteraction::BeamToFluidMortarManager::setup()
{
  ...
  lambda_dof_rowmap_ = std::make_shared<Core::LinAlg::Map>(
      -1, std::span<const int>(my_lambda_gid), 0, discretization_structure_->get_comm());
  ...
  Core::LinAlg::Map node_gid_rowmap(-1, std::span<const int>(my_nodes_gid.data(), n_nodes), 0,
      discretization_structure_->get_comm());
  Core::LinAlg::Map element_gid_rowmap(-1, std::span<const int>(my_elements_gid.data(), n_element),
      0, discretization_structure_->get_comm());
...
void BeamInteraction::BeamToFluidMortarManager::set_global_maps()
{
  ...
  beam_dof_rowmap_ = std::make_shared<Core::LinAlg::Map>(
      -1, std::span<const int>(field_dofs[0]), 0, discretization_structure_->get_comm());
  fluid_dof_rowmap_ = std::make_shared<Core::LinAlg::Map>(
      -1, std::span<const int>(field_dofs[1]), 0, discretization_fluid_->get_comm());
    ...
    const std::vector<std::shared_ptr<BeamInteraction::BeamContactPair>>& contact_pairs)
{
  ...
  Core::LinAlg::Map node_gid_needed_rowmap(
      -1, std::span<const int>(node_gid_needed), 0, discretization_structure_->get_comm());
  Core::LinAlg::Map element_gid_needed_rowmap(
      -1, std::span<const int>(element_gid_needed), 0, discretization_structure_->get_comm());
  ...
  lambda_dof_colmap_ = std::make_shared<Core::LinAlg::Map>(
      -1, std::span<const int>(lambda_gid_for_col_map), 0, discretization_structure_->get_comm());
